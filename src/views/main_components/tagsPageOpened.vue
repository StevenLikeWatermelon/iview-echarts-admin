...
export default {
    ...
    data () {
        return {
            currentPageName: (this.$route.meta.belong && !this.$route.meta.tmpNew) ? this.$route.meta.belong : this.$route.name,
            tagBodyLeft: 0,
    ...
    watch: {
        '$route' (to) {
            this.currentPageName = (to.meta.belong && !to.meta.tmpNew) ? to.meta.belong : to.name;
            this.$nextTick(() => {
                this.refsTag.forEach((item, index) => {
