            ...
            <Col span="24">
                <Card>
                    ...
                    <el-table :data="dataList" border stripe style="text-align: center;width: 100%"  max-height="570" @sort-change="manualSort" >
                        <el-table-column show-overflow-tooltip label="图片名称" prop="pic_name"></el-table-column>
                        <el-table-column show-overflow-tooltip label="图片路径" prop="pic_path">
                            <template slot-scope="scope">
                                <span style="color: rgb(45, 140, 240);cursor: pointer;" @click="showPicture(scope.row.pic_path, scope.row.pic_name)">{{scope.row.pic_path}}</span>
                            </template>
                        </el-table-column>
                            ...
                            <Row>
                                <Col span="24">
                                    <FormItem label="图片名称：" prop="pic_name">
                                        <Input v-model="addInfo.pic_name" placeholder="输入图片名称"></Input>
                                    </FormItem>
                                </Col>
                                <Col span="24">
                                    <FormItem label="图片上传：">
                                ...
                                <Col span="24">
                                    <FormItem label="链接地址：">
                                        <Input v-model="addInfo.url_path" placeholder="输入链接地址"></Input>
                                    </FormItem>
                                </Col>
                                <Col span="24">
...
<script type="text/ecmascript-6">
    import { apiAllStatus, mallCarouseListApi, mallCarouseAddApi, mallCarouseEditApi, mallCarouseDeleteApi } from '@/config/getData';
    ...
    export default {
        data () {
            return {
                ...
                uploadUrl: this.baseUrl + '/v1/mall/carousel-pic/upload?token=' + setToken(),
                spinShow: false,
                orderField: '',
                orderDirection: '',
                allStatusObj: [],
        ...
        methods: {
            ...
            editItem(index) {
                this.isAddModal = 0;
                this.addInfo.pic_name = this.dataList[index].pic_name;
            ...
            getStatus () {
                apiAllStatus(this, {
                    type: JSON.stringify(['carousel_status'])
                }).then(res => {
                    if (res && res.status == 1) {
                        //成功
                        this.allStatusObj = res.data.carousel_status;
                        let tmpObj = {};
                        this.allStatusArr = [];
                        for (let key in res.data.carousel_status) {
                            if (res.data.carousel_status.hasOwnProperty(key)) {
                                tmpObj = {
                                    value: key,
                                    label: res.data.carousel_status[key]
                                };
