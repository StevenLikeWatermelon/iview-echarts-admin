            ...
            <Col span="24">
                <Card>
                    ...
                    <el-table :data="dataList" border stripe style="text-align: center;width: 100%"  max-height="570" @sort-change="manualSort" >
                        <el-table-column show-overflow-tooltip label="楼层名称" prop="floor_name"></el-table-column>
                        <el-table-column show-overflow-tooltip label="楼层说明" prop="floor_desc"></el-table-column>
                        ...
                        <el-table-column show-overflow-tooltip label="操作">
                            <template slot-scope="scope">
                                <span style="color: rgb(45, 140, 240);cursor: pointer;" @click="editItem(scope.$index)">修改</span>
                                ...
                                <span style="color: rgb(45, 140, 240);cursor: pointer;margin-left: 15px;" v-if="scope.row.status == 0" @click="goToDetail(scope.row.id, 1)">详情编辑</span>
                                <span style="color: rgb(45, 140, 240);cursor: pointer;margin-left: 15px;" v-if="scope.row.status == 1" @click="goToDetail(scope.row.id)">详情查看</span>
                            </template>
                        </el-table-column>
                    </el-table>
                </Card>
                ...
                <col span="24">
                    <Card>
                        <Form :label-width="100" ref="addInfo" :model="addInfo" :rules="ruleValidateAddInfo">
                            <Row>
                                <Col span="24">
                                    <FormItem label="楼层名称：" prop="floor_name">
                                        <Input v-model="addInfo.floor_name" placeholder="输入楼层名称"></Input>
                                    </FormItem>
                                </Col>
                                <Col span="24">
                                    <FormItem label="楼层说明：" prop="floor_desc">
                                        <Input v-model="addInfo.floor_desc" placeholder="输入楼层说明"></Input>
                                    </FormItem>
                                </Col>
                                <Col span="24">
                                ...
                                <Col span="24" v-show="isAddModal == 0">
...
<script type="text/ecmascript-6">
    import { apiAllStatus, mallAllFloorsListApi, mallAllFloorsAddApi, mallAllFloorsEditApi, mallAllFloorsDeleteApi } from '@/config/getData';
    ...
    export default {
        data () {
            return {
                ...
                ruleValidateAddInfo: {
                    floor_name: [
                        { required: true, message: '输入楼层名称', trigger: 'blur' }
                    ],
                    floor_desc: [
                        { required: true, message: '输入楼层说明', trigger: 'blur' }
                    ],
                    sort: [
        ...
        methods: {
            goToDetail(id, isEdit) {
                util.openNewPage(this, 'floor_manage_detail', {
                    id,
                    isEdit
                });
                this.$router.push({
                    name: 'floor_manage_detail',
                    query: {
                        id,
                        isEdit
                    }
            ...
            addNew() {
                this.isAddModal = 1;
                this.addInfo.floor_name = '';
                this.addInfo.floor_desc = '';
            ...
            editItem(index) {
                this.isAddModal = 0;
                this.addInfo.floor_name = this.dataList[index].floor_name;
                this.addInfo.floor_desc = this.dataList[index].floor_desc;
            ...
            getStatus () {
                apiAllStatus(this, {
                    type: JSON.stringify(['floor_status'])
                }).then(res => {
                    if (res && res.status == 1) {
                        //成功
                        this.allStatusObj = res.data.floor_status;
                        let tmpObj = {};
                        this.allStatusArr = [];
                        for (let key in res.data.floor_status) {
                            if (res.data.floor_status.hasOwnProperty(key)) {
                                tmpObj = {
                                    value: key,
                                    label: res.data.floor_status[key]
                                };
