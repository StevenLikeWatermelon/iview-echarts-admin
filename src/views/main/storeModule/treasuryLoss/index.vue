        ...
        <Row>
            ...
            <Card>
                <Form :model="orderInfo" :label-width="120" class="model-style">
                    <Row>
                        <Col span="4">
                        <Input v-model="orderInfo.document_no" placeholder="输入盘亏出库单编号" @on-enter="search"></Input>
                        </Col>
                        <Col span="4">
        ...
        <Row class="margin-top-10 margin-pag-botm">
            ...
            <Card>
                <Row class="margin-bottom-10">
                    <Col span="24">
                    <Button type="primary" @click="packingAdd" v-if="is_current">新增盘亏出库单</Button>
                    <Button type="primary" @click="exportData" v-auth="exportButton" :class="{'margin-left-10': is_current}">导出</Button>
                    <span class="margin-left-10">盘亏金额总计：
                        <em style="color:#cf0012;font-style: normal;">{{ allTotal }}</em>
                    </span>
                    </Col>
                </Row>
                <el-table :data="orderData.data" border stripe style="text-align: center" @sort-change="manualSort" max-height="570">
                    <el-table-column prop="inventory_no" label="盘亏出库单编号" width="120" show-overflow-tooltip>
                        <template slot-scope="scope">
                            <span style="color: rgb(45, 140, 240);cursor: pointer" @click="handleClick({param: scope.row.inventory_no}, 'treasury-loss-detail')">{{scope.row.inventory_no}}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="store_name" label="盘亏仓库" min-width="120" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="area_name" label="盘亏库区" width="100" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="seat_name" label="盘亏库位" width="100" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="inventory_num" label="盘亏数量" sortable="custom" min-width="100"></el-table-column>
                    <el-table-column prop="inventory_price" label="盘亏单价" sortable="custom" min-width="100"></el-table-column>
                    <el-table-column prop="receipt_bank" label="盘亏金额" width="120">
                        <template slot-scope="scope">
                            <span>{{ scope.row.inventory_num * scope.row.inventory_price | toDecimal }}</span>
                        </template>
                    </el-table-column>
...
<script type="text/ecmascript-6">
    ...
    import { apiGetStockInventoryLossExport, apiAllStatus, apiGetStockInventoryLossList, operateAllBrandsList, apiGetAllStorageList, apiGetAllAreaList, apiGetAllSeatList, apiGetStockInventoryLossModel } from '@/config/getData';
    ...
    export default {
        ...
        data () {
            return {
                exportButton: 'packge_loss_export_button',
                orderInfo: {
                    document_no: '',
                    brand: 'all',
                    product_name: '',
        ...
        methods: {
            //导出功能
            exportData () {
                apiGetStockInventoryLossExport(this, {
            ...
            getData () {
                apiGetStockInventoryLossList(this, {
                    params: JSON.stringify(this.notempty({
                        numPerPage: this.limit,
                        pageNum: this.currentPage,
                        orderField: this.orderField,
                    ...
                    if(res && res.status == 1){
                        ...
                        this.allTotal = res.data.total_money;
                        this.spinShow = false;
                    } else {
            ...
            packingAdd () {
                this.handleClick('', 'treasury-loss-add');
            },
            search () {
                this.currentPage = 1;
                this.getData();
